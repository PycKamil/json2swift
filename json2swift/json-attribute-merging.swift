...
extension JSONType {
    ...
    private static func compatible(with type1: JSONType, and type2: JSONType) -> JSONType {
        ...
        case let (.date(r1, _), .url(r2)):     return .string(isRequired: r1 && r2, value: "")
        case let (.date(r1, f), .string(r2, _)):  return .date(  isRequired: r1 && r2, format: f)
        ...
        case let (.url(r1),     .date(r2, _)): return .string(isRequired: r1 && r2, value: "")
        case let (.url(r1),     .url(r2)):     return .url(   isRequired: r1 && r2)
        case let (.url(r1),     .string(r2, _)):  return .string(isRequired: r1 && r2, value: "")
        case     (.url,         .nullable):    return .url(   isRequired: false)
        case     (.nullable,    .url):         return .url(   isRequired: false)
        case let (.string(r1, _),  .date(r2, f)): return .date(  isRequired: r1 && r2, format: f)
        case let (.string(r1, _),  .url(r2)):     return .string(isRequired: r1 && r2, value: "")
        case let (.string(r1, value),  .string(r2, _)):  return .string(isRequired: r1 && r2, value: value)
        case     (.string,      .nullable):    return .string(isRequired: false, value: "")
        case     (.nullable,    .string):      return .string(isRequired: false, value: "")
